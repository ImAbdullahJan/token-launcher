...
$('#maxQuote').click(async function() {
  ...
  var getMaxQuote = await peerContract.methods.getMaxQuote(takerToken, makerToken).call().then((result) => {
    $('#maxQuoteDetails').text('Maximum Quote: Your can to buy ' + result[0] + " tokens of  to get " + result[1] + ' tokens of WETH.');
  }).catch((e) => {
    console.log(e);
  });
});

$('#getTakerTokens').click(async function() {

  var makerTokensign = $('#makerTokenAddress').val();
  console.log(makerTokensign);
  var takerTokensign = $('#takerTokenAddress').val();
  console.log(takerTokensign);

  var makerParam1 = $('#makerTokens').val();
  console.log(makerParam1);

  var getTakerSideQuote = await peerContract.methods.getTakerSideQuote(makerParam1, makerTokensign, takerTokensign ).call().then((result) => {
    $('#takerTokens').text( result );
    console.log(result);
